    ...
    def calculate_reward(self, state_before, state_after):
        ...
        if desired_pressure_bin - tolerance_bins <= state_after <= desired_pressure_bin + tolerance_bins:
            ...
            if improvement > 0:
                ...
                reward += improvement * 30  # Scale this factor to adjust sensitivity
            else:
...
def run_multiple_episodes(sim, q_agent, num_episodes):
    ...
    q_tables_over_time = []  # List to store Q-tables over time
    ...
    for episode in range(num_episodes):
        ...
        total_reward = 0
        q_agent.reset_action_frequencies()  # Reset action frequencies at the start of the episode
        while not done:
        ...
        q_agent.end_of_episode()  # Store action frequencies for the episode

        print(f"Episode {episode}: Total Reward = {total_reward}, Epsilon = {q_agent.epsilon:.4f}")

        # Calculate the cumulative average of the total rewards
    cumulative_avg = np.cumsum(total_rewards_per_episode) / np.arange(1, len(total_rewards_per_episode) + 1)

    # Plotting total rewards and cumulative average
    plt.figure(figsize=(12, 6))
    plt.plot(total_rewards_per_episode, label='Total Reward per Episode', alpha=0.5)
    plt.plot(cumulative_avg, label='Cumulative Average of Total Rewards', linewidth=2, color='red')
    plt.title("Total Rewards and Cumulative Average Over Episodes")
    plt.xlabel("Episode")
    plt.ylabel("Rewards")
    plt.legend()
    plt.grid(True)
...
num_episodes = 10  # for example, run for 100 episodes
...
def plot_action_distribution_per_episode(q_agent):
    # Assuming the action indices start from 1 and ignoring index 0
    num_episodes = len(q_agent.episode_actions)
    actions = range(1, q_agent.num_actions + 1)
    frequencies_per_episode = {action: [] for action in actions}

    for episode in q_agent.episode_actions:
        for action in actions:
            frequencies_per_episode[action].append(episode[action])

    # Now plot the data
    plt.figure(figsize=(12, 8))
    for action, freq in frequencies_per_episode.items():
        plt.plot(freq, label=f'Action {action}')

    plt.title('Action Distribution Per Episode')
    plt.xlabel('Episode')
    plt.ylabel('Frequency of Action Taken')
    plt.legend()
    plt.grid(True)
    plt.show()


plot_action_distribution_per_episode(q_agent)
