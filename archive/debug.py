...
class GasPipeNetworkSimulation:
    ...
    def add_strategic_source(self, location_id):
        ...
        source_idx = ppipe.create_source(self.net, junction=junction_idx, mdot_kg_per_s=5,
    ...
    def calculate_reward(self, state_before, state_after):
        desired_pressure_bin = 250
        tolerance_bins = 5
        max_possible_deviation = 250

        # Base reward for being within the tolerance
        reward = 10 if desired_pressure_bin - tolerance_bins <= state_after <= desired_pressure_bin + tolerance_bins else 0
        ...
        # Symmetric reward/penalty for improvement/regression
        reward_change = improvement * 20  # Adjust scale as needed
        reward += reward_change

        # Ensure reward is within reasonable bounds
        reward = max(min(reward, 20), -20)  # Adjust bounds as needed
...
num_episodes = 100  # or however many episodes you wish to run
total_rewards_per_episode, q_tables_over_time, action_frequencies_over_time = run_multiple_episodes(sim, q_agent, num_episodes)


# Now that you have the data, call the visualization functions
plot_selected_state_q_values(q_tables_over_time, 220, 225, num_episodes)  # Pass q_tables_over_time directly
# For the learning curve, simply pass the list of total rewards per episode
plot_learning_curve(total_rewards_per_episode)

# For action selection frequency, pass the action frequency data over time
plot_action_selection_frequency_over_time(action_frequencies_over_time)

# You may need to modify your simulation code to apply the learned policy from the Q-table
pressures_after = sim.get_pressures()

# Plot pressures before and after Q-learning
sim.plot_pressures(pressures_before, pressures_after)