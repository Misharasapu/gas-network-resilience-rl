...
def run_episode(sim):
    ...
    while not done and steps < max_steps:
        # This example assumes you have a function or method to select an action
        # For simplicity, let's say we're always using the action to add a strategic source
        action = 1  # Your action selection mechanism goes here

        # Perform the action
        sim.step(action)

        # Check the new state of the environment
        state_after = sim.get_state()

        # Determine if the episode should end based on the current state
    ...
    if done:
        ...
        print(f"Episode reached the maximum of {max_steps} steps without completion.")
