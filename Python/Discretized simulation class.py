...
class GasPipeNetworkSimulation:
    ...
    def get_state(self):
        """
        Returns the current state of the environment, discretized into bins.
        Each bin represents a range of pressure values.
        """
        self.run_simulation()  # Ensure the latest state is reflected

        # Define the pressure range and bin size
        bin_size = 0.1  # in bar, defining the precision of the discretization
        min_pressure = 0  # minimum expected pressure in the network
        max_pressure = 25  # maximum expected pressure in the network
        bins = np.arange(min_pressure, max_pressure + bin_size, bin_size)

        # Get continuous pressure values
        continuous_pressures = self.net.res_junction.p_bar.values

        # Discretize pressures: np.digitize returns the indices of the bins to which each value belongs
        discrete_pressures = np.digitize(continuous_pressures, bins) - 1  # -1 to start bin index at 0

        # Ensure the state length is fixed
        max_junctions = 50  # maximum number of junctions expected
        state = np.zeros(max_junctions, dtype=int)  # initialize state with zeros

        # Fill the state array with discretized pressures
        state[:len(discrete_pressures)] = discrete_pressures[:max_junctions]
    ...
    def calculate_reward(self, state_before, state_after):
        ...
        reward = 0
        for i, pressure_bin in enumerate(state_after):
            if desired_pressure_bin - tolerance_bins <= pressure_bin <= desired_pressure_bin + tolerance_bins:
                reward += 5  # Increase reward for each junction within the desired range
            else:
                deviation_after = abs(pressure_bin - desired_pressure_bin)
                deviation_before = abs(state_before[i] - desired_pressure_bin)
                ...
                normalized_deviation_before = deviation_before / max_deviation
                ...
                if deviation_after < deviation_before:
                    ...
                    reward -= min(normalized_deviation_after * 5,
                                  10)  # Cap the penalty to prevent excessively large values

        return reward

    def check_if_done(self, state):
        # The reference pressure is 25 bar, and each bin represents 0.1 bar
        reference_pressure_bin = 250  # 25 bar
        tolerance = 0.1  # 10%

        # Calculate tolerance in terms of bins
        tolerance_bins = int(tolerance * reference_pressure_bin)

        # Check if all pressures are within the acceptable range
        all_within_range = all(reference_pressure_bin - tolerance_bins <= p <= reference_pressure_bin + tolerance_bins
                               for p in state if p > 0)  # Assuming 0 is used for uninitialized/unused bins
    ...
    def step(self, action):
        ...
        Returns:
        - state_after (ndarray): The new state of the environment after executing the action.
...
def run_episode(sim):
    ...
    while not done and steps < max_steps:
        ...
        if done:
            ...
            print(f"After {steps} steps, goal not yet reached.")

    if not done:
        print("Maximum steps reached without fulfilling goal.")
    else:
        print(f"Episode successfully completed in {steps} steps.")
